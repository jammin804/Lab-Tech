...
func _on_pressed() -> void:
	if skill.cost <= SaveLoad.SaveFileData.money and is_upgradable() and not enabled:
		SaveLoad.SaveFileData.money -= skill.cost
		enabled = true
		#SaveLoad.SaveFileData.set_and_save()
...
func on_mouse_entered() -> void:
	...
	if enabled:
		...
		if is_upgradable() and not enabled and skill.cost <= SaveLoad.SaveFileData.money:
			description.text += "\n Can Purchase"
